...
Use `--local-yaml` to install from a local operator manifest instead of
downloading it. The operator creates and updates the CSI workloads; there is no
separate CSI manifest argument.

[source,console]
----
kubectl kadalu install --local-yaml=./kadalu-operator.yaml
----

...
`kadalu.<volname>` (`volname` will be replaced with `e-vol`) by default. Set
`spec.storageClassName` on the `KadaluStorage` to choose a different immutable
name. This does not mark the generated class as the cluster's default, even if
the chosen name is `default`.
...
Then run `kubectl apply -n kadalu -f ./external-gluster-as-pv.yaml`, replacing
`kadalu` with the namespace where the operator is installed when needed.

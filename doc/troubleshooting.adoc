...
== CSI pods do not start

The Kadalu operator applies the CSI controller and node resources after it
starts. If only the `operator` pod is present, inspect its logs and Kubernetes
events before deleting or restarting anything:
...
$ kubectl logs -n kadalu deployment/operator
$ kubectl get events -n kadalu --sort-by=.lastTimestamp
----

Confirm that the stable CSI registration object and workloads were applied:

[source,console]
----
$ kubectl get csidriver kadalu
$ kubectl get daemonset,statefulset -n kadalu -l app.kubernetes.io/part-of=kadalu
----

The operator uses idempotent declarative apply, so an existing `CSIDriver`
normally should not be deleted. Resolve the concrete authorization, validation,
or image-pull error shown in the logs or events instead.
...
Reapply the appropriate operator manifest from the
https://github.com/joejulian/kadalu/releases/latest[latest fork release], then
restart the operator so it reconciles the CSI workloads. Verify that the
`kadalu-csi-nodeplugin` DaemonSet is available on the affected node before
retrying the workload.

[source,console]
----
kubectl apply -f kadalu-operator.yaml
kubectl -n kadalu rollout restart deployment/operator

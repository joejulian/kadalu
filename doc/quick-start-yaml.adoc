...
[source,console]
----
$ kubectl apply -f https://github.com/joejulian/kadalu/releases/latest/download/kadalu-operator.yaml
----

You have different flavors of operator yaml files available. Mainly for kubernetes (default), openshift, RKE, Microk8s. Find the relevant file from https://github.com/joejulian/kadalu/tree/devel/manifests[here]. Just remember to use the raw file as argument to `kubectl`.
...
[source,console]
----
$ kubectl apply -n kadalu -f sample-storage-config.yaml
----

Use the namespace where the operator is installed in place of `kadalu` for a
custom-namespace deployment. `KadaluStorage` is namespaced and must be created
where that operator watches.
